...
if (game) {
  ...
  for (let i = 1; i <= nbPlayer; i++) {
    ...
    const polisName = readlineSync.question('May I have your Polis\'s name? ');
...
while (game && (roundCounter < nbRound)) {
  players.forEach(player => {
    ...
    switch (playerChoice) {
      case 1: {
        ...
        numInt =
          readlineSync.questionInt(player.color('Enter a number :'),
            'limitMessage');
        for (let i = 0; i < numInt; i++) {
          player.polis.createLightSoldier();
        }
        console.log(
        ...
        numInt =
          readlineSync.questionInt(player.color('Enter a number :'),
            'limitMessage');
        for (let i = 0; i < numInt; i++) {
          player.polis.createHeavySoldier();
        }
        break;
      }
      case 2: {
        console.log(
          player.color('How many Rider do you want to put in defense ? '));
        const numR = readlineSync.questionInt(player.color('Enter a number :'),
          'limitMessage');
        console.log(numR);
        console.log(player.color(
          'How many Light Soldier do you want to put in defense ? '));
        const numL = readlineSync.questionInt(player.color('Enter a number :'),
          'limitMessage');
        console.log(numL);
        console.log(player.color(
          'How many Heavy Soldier do you want to put in defense ? '));
        const numH = readlineSync.questionInt(player.color('Enter a number :'),
          'limitMessage');
        console.log(numH);
      ...
      case 3: {
        console.log(
          player.color('How ɱany corn do you want to put in this trade ? '));
        const numCorn = readlineSync.questionInt(
          player.color('Enter a number :'),
          'limitMessage');
        console.log(numCorn);
        console.log(
          player.color('How ɱany gold do you want to put in this trade ? '));
        const numGold = readlineSync.questionInt(
          player.color('Enter a number :'),
          'limitMessage');
        console.log(numGold);
        const polisTrade = readlineSync.keyInSelect(otherPolis, player.color(
          'Which city do you want to do business with?'));
        player.polis.doTradeWith(otherPlayer[polisTrade].polis);
        break;
      }
      case 4: {
        const polisattack = readlineSync.keyInSelect(otherPolis, player.color(
      ...
      case 5: {
        ...
        switch (polisResource) {
          case 0:
            ...
            numInt = readlineSync.questionInt(player.color('Enter a number :'),
              'limitMessage');
